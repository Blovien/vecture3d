...
// Projects the field onto a 2D grid by flattening its thinnest axis. Each projected
// cell stores references to every hitbox that overlaps it, supporting caves, decks,
// and overhangs. A hitbox may therefore appear in more than one cell.
//
// Rays visit the projected cells they cross. The fallback group walk visits groups
// in the ray's bounding box. A zero cellCount selects the fallback.
//
// Cell coordinates start at zero so placement does not affect the content hash.
...
// The cooked header starts the allocation. Fields before V3_BLOCK_GRID_CONTENT_OFFSET
// hold mutable reference and placement data. Content from byteCount onward is immutable.
// The hash excludes the mutable fields so identical geometry cooked at different origins
// has the same hash. Payload coordinates are grid local. Entry points translate them.
struct v3BlockGridData
{
	...
	// Reduction counts satisfy inputBoxCount = hitboxCount + culledBoxCount + mergedBoxCount.
