...
#error "v3_interval.h requires FLT_EVAL_METHOD == 0; build with SSE2 or better, never x87"
#endif
...
// The summation order is normative so results stay deterministic
...
// Refuses to certify on a platform whose arithmetic breaks the inflation model
// The world calls this once and a failure downgrades every rotational query to
// an uncertain outcome, so the certifier declines to run rather than lie
