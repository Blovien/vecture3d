...
// What reduction knows about one occupied cell.
//
// A cell is solid when its whole Cell Box set is exactly the unit cube: one box, filling the
// cell and nothing more. That is the only shape reduction reasons about, because it is the only
// one whose face covers a neighbour's face exactly and whose union with a neighbour is again a
// box. A cell holding several boxes, or one sub-cell box, keeps every box it was given.
// Growing a run tests thousands of cells, so everything a test reads lives here rather than in
// the box list, which is an order of magnitude larger and would cost a cache miss per test.
...
// The cell lookup is open addressing over a power-of-two capacity with no deletion, so a
// neighbour test costs about one probe and reduction scales with the block count rather than the
// frame volume a dense lattice would pay for even where the field is empty.
//
// One slot is one word, the packed grid-local coordinate above the block index it names, and
// zero for empty. Reduction probes the table around ten times per block, so keeping a slot to
// eight bytes is what keeps the whole table inside the cache for any field the cooker accepts.
...
static bool v3BlockGridAllocateReductionScratch( v3BlockGridCook* cook )
{
	...
	// Three quarters full at worst, so the probe chain stays short without a rehash and the table
	// stays small enough to sit in cache.
	int capacity = 16;
...
// Replaces the surviving solid cells by the smallest set of slabs that covers exactly the same
// solid, then keeps every other box untouched.
//
// Cells are visited in cooked block order, so which cell starts which run is a function of the
// content alone and the same field always reduces to the same slabs. A run inherits its origin
// cell's identity, which stays unique because it is one of the identities the input already
// proved unique.
...
// Drops the geometry nothing can touch and merges what is left, so the cooked field carries the
// fewest boxes that still describe exactly the same solid. Logical cells are untouched, which is
// why mass and inertia cannot move.
...
// The maximum size of the projection grid, as a multiple of the hitbox data it
// makes faster.
//
// The limit counts bytes, not references. The grid also pays for empty cells.
// An earlier limit counted only references. It accepted a field of four cubes
// in a large frame, and that grid used 2 MB for empty cells.
...
// Finds the cells that a hitbox covers. The result is { u0, v0, u1, v1 } and
// includes both limits. The range is empty when u1 is less than u0.
//
// The upper edge is open. A box that stops right on a cell edge does not use
// the next cell. A run of four blocks thus covers four cells, not five.
//
// The group index rounds the upper edge differently. An earlier version used
// the group rule here. It made almost four references for each unit cube, but
// one reference was correct.
//
// The two indexes can round differently, because each one is complete on its
// own. Each hitbox is in all the cells that it covers, so a ray cannot pass
// between them.
...
// Selects the axis to flatten. This is always the thinnest axis.
//
// The flattened axis loses its detail, and the thinnest axis has the least
// detail to lose. A fixed vertical axis is correct for terrain but wrong for a
// tower. The long axis was several times slower in tests.
...
static bool v3BlockGridEmit( v3BlockGridCook* cook )
{
	...
	// The payload frame is grid-local, so its origin is always zero. The world
	// origin the caller cooked at lives in the mutable trailer.
