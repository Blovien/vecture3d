...
//
// Engine-side adapters for the static BlockGrid shape.
//
...
// Mass comes from the logical blocks rather than the collision hitboxes, and
// because every block is a unit cube of uniform density the sum is exact instead
// of sampled, since the centre of mass is simply the mean block centre while the
// inertia is one cube's tensor plus a parallel axis shift, accumulated over the
// field.
//
// Hitboxes look like the obvious input and are the wrong one, since they
// describe the collision surface instead of the material. A merged run covers
// many blocks with a single box, a stair contributes two boxes for one block,
// and a fence panel is mostly air, so any of those would weigh the assembly
// wrong.
//
// Calculations use normalized stored cells. The returned centroid restores the
// cook origin to match the caller's local frame. Inertia about the centroid is
...
// The paths below narrow the field with one grid query and then run the ordinary
// convex function for each candidate the query reports.
//
// They test candidates as the query finds them rather than collecting them
// first. An earlier version gathered indices into a fixed array of 256 and
// stopped storing beyond that while still counting, so a query returning more
// simply lost the rest. A cast over a 24 by 24 sheet reported 600 candidates,
// kept 256, and travelled through a solid bar that sat among the 344 it
// discarded. Streaming has no such ceiling, and the query already visits each
// hitbox once.
											  ...
											  int candidateCap )
{
	...
	for ( int i = 0; i < work.candidateCount; ++i )
	{
		...
		{
			// This candidate can still be reached sooner than the best impact found,
			// and the cap forbids finding out. Accepting that impact could put the
			// body past this Hitbox, so the sweep has no answer and the fast body
			// holds here instead.
			result.capExhausted = true;
...
b3AABB v3ComputeBlockGridAABB( const v3BlockGridData* grid, b3Transform transform )
{
	...
	// A BlockGrid is axis aligned and static, but the body may still carry a
	// rotation, so transform all eight corners rather than the two extremes.
