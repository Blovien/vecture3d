...
#define V3_MAX_BLOCK_CONTACT_EVENTS V3_BLOCK_CONTACT_EVENT_CAPACITY
...
static inline bool v3_body_flags_are_valid( uint32_t kind, uint32_t flags )
{
	if ( ( flags & ~( V3_BODY_ENABLE_SLEEP | V3_BODY_INITIAL_AWAKE | V3_BODY_DISABLE_COLLISION | V3_BODY_BULLET ) ) != 0 )
	{
		return false;
	}

	return kind == V3_DYNAMIC_BODY || ( flags & V3_BODY_BULLET ) == 0;
}
...
typedef struct v3_body_identity
{
	b3BodyId body_id;
	uint64_t logical_id;
	uint32_t generation;
...
typedef struct v3_block_contact_storage
{
	...
	uint32_t flags;
	uint32_t previous_body_identity_count;
} v3_block_contact_storage;
...
struct v3_world
{
	...
	v3_block_contact_storage* block_contacts;
};
...
typedef enum v3_test_fault
{
	...
	V3_TEST_FAULT_CREATE_JOINT,
	V3_TEST_FAULT_BLOCK_CONTACT_STORAGE_CALLOC,
} v3_test_fault;
