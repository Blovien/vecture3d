...
// BlockGrid pair behavior at the public seam: create shapes, step the world, and read contacts,
// counters, and recordings back through the published API. Nothing here reaches into the update.
...
// The same column on a BlockGrid floor, so the pair path resolves both Hitbox materials.
//
// Same fixture, same locks and same derivation as the case above: the floor is level and carries a
// single material of friction 1.0, the column rests on one Hitbox, and mu = sqrt(faceFriction * 1.0)
// gives 0.5 and 1.0, so after t = 0.5 s at g = 10 the speed left of v0 = 8 m/s is 5.5 and 3.0 m/s
// within the same 0.5 m/s tolerance. What differs is only that the floor is a grid, so the contact
// is a BlockGrid pair and its materials come from v3BlockGridPairResolvePatchMaterial rather than
// from the grid-to-convex branch. The floor spans ten cells from x = -2, well past the 4 m the
// column can cover in the window.
...
static int V3BlockGridPairCountersBehavior( void )
{
	...
	// The three counters whose producers arrive with later tickets read zero until then
...
// ---------------------------------------------------------------------------------------------
// Cooked geometry reduction at the public seam: cook through v3CookBlockGrid, attach, step, and
// read the cooked statistics and the resulting poses back. Every expectation below is derived in
// its own comment from the definition, never taken from a run.
// ---------------------------------------------------------------------------------------------
...
static int V3BlockGridShuffledDefinitionCooksIdentically( void )
{
	...
	// The whole cooked allocation, byte for byte. Both cooks are compared before either is
	// attached or retained, so even the mutable leading trailer holds the same one reference and
	// the same cooking origin, and the comparison covers the payload the content hash is taken
	// over as well as everything around it.
