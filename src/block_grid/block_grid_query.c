...
// Traverses projected cells crossed by the ray and tests their hitboxes.
// Equal hit distances use the lower source ID, matching the group walk.
// Returns false when no projection grid exists, requiring the caller to use the group walk.
										  ...
										  float* bestFractionOut, int* bestAxisOut, float* bestSignOut )
{
	...
	// At the starting cell edge, test both neighboring cells so hits at distance zero
	// participate in the tie rule. Step back one cell when the ray leaves that edge behind.
	// Later traversal already visits both sides of each crossed edge.
