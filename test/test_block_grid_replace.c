...
// BlockGrid revision replacement tests observe identity, bounds, mass, contacts,
// counters, and allocation accounting through the published API.
...
// Removing blocks under one ship ends its contact on the next step. The other ship's
// contact is rebuilt. Compare that ship's pose and velocity at each step with an
// unreplaced control world using the tolerances below.
static int ReplaceEndsRemovedBlockContacts( void )
{
	...
	// Rebuilding contact geometry moves the resting ship by less than the linear slop.
