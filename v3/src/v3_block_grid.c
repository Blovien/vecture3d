...
// One cooking handle owns exactly one reference to immutable cooked data. Box3D carries density per
// shape rather than per material, so the density every shape this data backs is created with is
// resolved once here, at cook time, from material zero.
