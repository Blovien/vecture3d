...
static v3BlockGridPairUpdateScratchLayout v3BlockGridPairPlanUpdateScratch( void )
{
	...
	// Preserve candidate scratch alignment and byte count from the initial policy version 2 implementation.
...
// Every step that produces no usable geometry ends the same way an ordinary Box3D contact does when
// its manifold function reports no touch: this contact keeps its identity and loses its manifolds
														  ...
														  v3BlockGridPairUpdateScratch* scratch )
{
	...
	// A rebuild that fits the retained owner keeps it, so a warmed-up step reaches no allocator
